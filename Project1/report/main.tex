...
% In the previous expressions, we have assumed that the measurements of our dataset is independent of one another, but what happens if they are correlated? Here, we assume we have a time series, in which each Monte Carlo step is equivalent to one step in time. For uncorrelated results we have that the standard deviation is
% \begin{align}
%     \sigma = \sqrt{\frac{1}{n-1}(\langle x^2 \rangle - \langle x \rangle ^2)}. \label{eq:sigma_uncorrelated}
% \end{align}

% However, when dealing with correlation, which we under reasonable suspicion can assume we have in our dataset, we have

% \begin{align}
%     \sigma = \sqrt{\frac{1 + 2\tau/\Delta t}{n-1}(\langle x^2 \rangle - \langle x \rangle ^2)} \label{eq:sigma_correlated}
% \end{align}
% where $\tau$ is the so-called \textit{autocorrelation time}, i.e. the time between an arbitrary sample and the next uncorrelated sample, whereas $\Delta t$ is the time between one sample and the next. The definition of autocorrelation is essentially the similarity of measurements separated by a distance $d$. Now, this distance can be used to reflect the correlation between measurements in a value known as $f_d$, where

% \begin{align}
%     f_d = \frac{1}{nm}\sum_{\alpha=1}^m\sum_{kl=1}^n(x_{\alpha, k} - \langle x_m \rangle)(x_{\alpha, l} - \langle x_m \rangle). \label{eq:f_d}
% \end{align}
% The autocorrelation function can then be expressed as
% \begin{align}
%     k_d = \frac{f_d}{\sigma^2} \label{eq:autocorrelation},
% \end{align}
% with the autocorrelation time
% \begin{align}
%     \tau = 1+2\sum_{d=1}^{n-1}k_d\label{eq:autocorrelation_time}.
% \end{align}

% In order to find a good estimate on the correlation between measurements we can use a method known as \textit{blocking}\footnote{This is further described in section~\ref{subsubsec:blocking} under~\ref{sec:Methods}.~\nameref{sec:Methods}.}.


% To determine how well these estimates approximate the true mean value, we can calculate the sample error, which is just the standard deviation of the sample means. Let's look at a specific block $(\vec{X}_i)_k$, denoted by $X$ for brevity. The sample error is given by \cite{notes}
...
% We wish to scale this Hamiltonian so lengths are expressed in units of the characteristic length of the trap, $a_{ho}$, and energy in units of $\hbar \omega_{ho}$. 
